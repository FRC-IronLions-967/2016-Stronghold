...
//    	leftRotate = new CANTalon(6);
    ...
    public boolean climberToExtended(){
    	...
    	double count = 0;
//    	int done = 0;
    	if(leftExtention.getEncPosition() < -20000){//1000 should be the upright encoder value
    		left = 0;
//    		done = done+1;
    	}
    	if(rightExtention.getEncPosition() > 20000){
    		right = 0;
    	}
    	if(count < 1){
    		climberMoveSeperate(.9, .9);
    		left = .9;
    		right = .9;
    	}
    	else{
    		climberMoveSeperate(left, right);
    	}
    	count ++;
    	if(left == 0 && right == 0){
    		return true;
